...
void GAME_OBJ::setObjSpace(Plane *a_Plane, int a_Num)
{
	mNumHalfPlanes = a_Num;
	for (int i =0; i < mNumHalfPlanes; i++) {
		mHalfPlane[i] = a_Plane[i];
	}
}
...
{
	mLifeTime = cLife;
if (cMaxLife > 0) mMaxLifeTime = cMaxLife;
}
